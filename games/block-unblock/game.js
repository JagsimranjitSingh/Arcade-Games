...
function lerpColor(c, t, target = 0xffffff) {
...
class GameScene extends Phaser.Scene {
	...
	_buildBG() {
		...
		for (let x = 20; x < W; x += 32) for (let y = 20; y < H; y += 32) {
			dots.fillStyle(0xffffff, 0.022); dots.fillCircle(x, y, 1.2);
		}

		const cells = this.add.graphics();
		for (let r = 0; r < ROWS; r++) for (let c = 0; c < COLS; c++) {
			cells.fillStyle(0xffffff, 0.028);
		...
		for (let y = GY; y < GY + ROWS * CELL; y += 8) { sc.lineStyle(1, 0xffffff, 0.015); sc.lineBetween(GX, y, GX + COLS * CELL, y); }
	...
	_buildHUD() {
		...
		this.add.text(W / 2, 30, 'BLOCK  UNBLOCK', { fontSize: '24px', fontFamily: "'Courier New',monospace", color: '#ffffff', letterSpacing: 9 }).setOrigin(0.5);
		...
		[{ label: 'LEVEL', x: GX }, { label: 'MOVES', x: W / 2 - bxW / 2 }, { label: 'SCORE', x: GX + COLS * CELL - bxW }].forEach(({ label, x }) => {
			...
			bg2.fillStyle(0xffffff, 0.03); bg2.fillRoundedRect(x, boxY, bxW, boxH, 6);
		...
		this.scoreText = this.add.text(GX + COLS * CELL - bxW + 10, boxY + 28, '00000', { ...tcfg, color: '#ffffff' });
