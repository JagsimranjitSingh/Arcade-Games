...
(function () {
	...
	class GameScene extends Phaser.Scene {
		...
		_buildHUD() {
			...
			this.hintTxt = this.add.text(W / 2, H - 26, hint, sf('12px', '#ffffff'))
		...
		_die() {
			...
			if (!this.revived) {
				...
				const txtSkip = this.add.text(cx + 75, cy + 30, 'SKIP', { fontFamily: '"Share Tech Mono", monospace', fontSize: '16px', color: '#fff' }).setOrigin(0.5).setDepth(202);
		...
		create() {
			...
			if (sessionBest > this.score) {
				this.add.text(W / 2, H / 2 + 58,
					'BEST  ' + sessionBest.toLocaleString() + ' m', sf('11px', '#ffffff'))
			...
			if (isTouchDevice()) {
				this.add.text(W / 2, H / 2 + 164,
					'TAP ANYWHERE TO RESTART', sf('11px', '#ffffff'))
